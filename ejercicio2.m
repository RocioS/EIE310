...
N = 204800; n = [0 : N-1]; 
x =  0.08 * cos(2*pi * 4900   * n/N) + ...  
            cos(2*pi * 5160   * n/N) + ... 
            cos(2*pi * 5230.6 * n/N) + ... 
    0.003 * cos(2*pi * 6000   * n/N); 
...
xm = x .* hamming(N)';  
M = mean(hamming(N)); 
% factor de atenuación de la ventana
...
subplot 221; k = [0 : N-1]; semilogy(k, Mr); axis([4000 6500 0.001 1]); 
grid; title('Rectangular'); ylabel('Mag') 
subplot 222; semilogy(k, Mm); axis([4000 6500 0.001 1]); 
grid; title('Hamming'); ylabel('Mag') 
subplot 223; semilogy(k, Mn); axis([4000 6500 0.001 1]); 
grid; title('Hanning'); xlabel('k'); ylabel('Mag') 
subplot 224; semilogy(k, Mb); axis([4000 6500 0.001 1]); 
